defmodule HexHub.PackagesTest do
  ...
  describe "package management" do
    ...
    test "create_package/4 validates package name format" do
      ...
      assert {:error, _reason} = Packages.create_package("Invalid", "hexpm", %{"description" => "Test"})
      assert {:error, _reason} = Packages.create_package("123invalid", "hexpm", %{"description" => "Test"})
    end
  end
  ...
  describe "release management" do
    test "create_release/5 creates a new release" do
      ...
      assert {:ok, release} = Packages.create_release(package_name, version, meta, requirements, tarball)
    ...
    test "list_releases/1 returns all releases for a package" do
      ...
      {:ok, _} = Packages.create_release(package_name, "1.1.0", %{"app" => "test_package"}, %{}, "tarball2")
    ...
    test "create_release/5 validates version format" do
      ...
      assert {:error, _reason} = Packages.create_release(package_name, "invalid", %{}, %{}, "tarball")
    end
    ...
    test "upload_docs/3 and delete_docs/2 work correctly" do
      ...
      version = "1.0.0"
      {:ok, _} = Packages.create_package(package_name, "hexpm", %{"description" => "Test"})
      {:ok, _} = Packages.create_release(package_name, version, %{"app" => "test_package"}, %{}, "tarball")
