defmodule HexHub.Packages do
  ...
  """
  @spec create_package(String.t(), String.t(), map(), boolean()) :: {:ok, package()} | {:error, String.t()}
  ...
  def get_package(name) do
    case :mnesia.transaction(fn ->
      ...
      {:atomic, [{@packages_table, name, repository_name, meta, private, downloads, inserted_at, updated_at, html_url, docs_html_url}]} ->
        {:ok, package_to_map({@packages_table, name, repository_name, meta, private, downloads, inserted_at, updated_at, html_url, docs_html_url})}
      {:atomic, []} ->
        {:error, :not_found}
      {:aborted, reason} ->
        {:error, :not_found}
  ...
  def list_packages() do
    case :mnesia.transaction(fn ->
      :mnesia.foldl(
        fn {_, name, repository_name, meta, private, downloads, inserted_at, updated_at, html_url, docs_html_url}, acc ->
          [package_to_map({@packages_table, name, repository_name, meta, private, downloads, inserted_at, updated_at, html_url, docs_html_url}) | acc]
        end,
        [],
        @packages_table
      )
  ...
  """
  @spec create_release(String.t(), String.t(), map(), map(), binary()) :: {:ok, release()} | {:error, String.t()}
  ...
  def get_release(package_name, version) do
    case :mnesia.transaction(fn ->
      :mnesia.read(@releases_table, {package_name, version})
    end) do
      {:atomic, [{@releases_table, package_name, version, has_docs, meta, requirements, retired, downloads, inserted_at, updated_at, url, package_url, html_url, docs_html_url}]} ->
        {:ok, release_to_map({@releases_table, package_name, version, has_docs, meta, requirements, retired, downloads, inserted_at, updated_at, url, package_url, html_url, docs_html_url})}
      {:atomic, []} ->
        {:error, :not_found}
      {:aborted, reason} ->
        {:error, :not_found}
  ...
  def list_releases(package_name) do
    case :mnesia.transaction(fn ->
      :mnesia.match_object({@releases_table, package_name, :_, :_, :_, :_, :_, :_, :_, :_, :_, :_, :_, :_})
    end) do
      {:atomic, releases} ->
      ...
      {:ok, release} ->
        ...
        case Storage.upload(docs_key, docs_tarball) do
          {:ok, _} ->
            ...
            case :mnesia.transaction(fn ->
              case :mnesia.read(@releases_table, {package_name, version}) do
                [{@releases_table, package_name, version, _has_docs, meta, requirements, retired, downloads, inserted_at, _updated_at, url, package_url, html_url, docs_html_url}] ->
                  updated_release = {
                    @releases_table,
                    package_name,
                    version,
  ...
  defp package_to_map({@packages_table, name, repository_name, meta, private, downloads, inserted_at, updated_at, html_url, docs_html_url}) do
    %{
      name: name,
  ...
  defp release_to_map({@releases_table, package_name, version, has_docs, meta, requirements, retired, downloads, inserted_at, updated_at, url, package_url, html_url, docs_html_url}) do
    %{
  ...
  defp validate_package_name(name) do
    cond do
      String.length(name) < 1 -> {:error, "Package name must not be empty"}
      String.length(name) > 100 -> {:error, "Package name must be at most 100 characters"}
      not String.match?(name, ~r/^[a-z][a-z0-9_]*$/) -> {:error, "Package name must start with lowercase letter and contain only lowercase letters, numbers, and underscores"}
      true -> :ok
    end
  end
  ...
  defp validate_version(version) do
    cond do
      String.length(version) < 1 -> {:error, "Version must not be empty"}
      not String.match?(version, ~r/^\d+\.\d+\.\d+.*$/) -> {:error, "Version must be in semantic versioning format"}
      true -> :ok
    end
  end
end